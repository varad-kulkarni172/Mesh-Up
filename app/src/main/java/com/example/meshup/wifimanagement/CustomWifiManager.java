			...
			final int reachableTimeout, final FinishScanListener finishListener)
	{
		Runnable runnable = new Runnable()
		{
			public void run()
			{
				...
				final ArrayList<VisibleClient> result = new ArrayList<VisibleClient>();
					...
					while ((line = br.readLine()) != null)
					{
						...
						if ((splitted != null) && (splitted.length >= 4))
						{
							...
							if (mac.matches("..:..:..:..:..:.."))
							{
								...
								{
									result.add(new VisibleClient(splitted[0],
		...
		Log.d("Meshup Debug", "CWM getClientList: Now waiting...");
		
		try
		{
			myThread.join();
			
			Log.d("Meshup Debug", "CWM getClientList: Now resuming...");
		} 
